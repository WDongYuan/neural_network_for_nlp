...
class TwoPointerModel(nn.Module):
	def __init__(self,embedding_size,hidden_size,direction,word_em,batch_size,context_max_length,question_max_length,max_answer_length):
		super(TwoPointerModel, self).__init__()
		self.name = "two_pointer_model"
		...
		self.end_att_linear = nn.Linear(self.question_max_length,1)
	...
	def forward(self,question,passage,start_point=None):
		...
		answer_question_align = torch.bmm(answer_range,torch.transpose(q_lstm_out,1,2))
		end_align_score = self.end_att_linear(answer_question_align)
		end_pro = self.softmax(end_align_score.view(self.batch_size,-1))


		return start_pro,end_pro,passage_length
